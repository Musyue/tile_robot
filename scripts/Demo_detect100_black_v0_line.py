...
class DetectTile:
    ...
    def select_rgb_white_yellow(self,image):
        ...
        return masked
    def select_rgb_black(self,image):
        # white color mask
        lower = np.uint8([0, 0, 0])
        ...
        for cont in contours:
            ...
            # if cv2.contourArea(cont) > 3000:
                if (len(approx) == 4):
                    ...
                    new_approx=sorted(new_approx, reverse=True, key=lambda k: [k[1], k[0]])
                    # new_approx=approx
                    ...
                    cv2.line(rgb, ((angular_point[0][0]+angular_point[2][0])/2,(angular_point[0][1]+angular_point[2][1])/2), (cX,cY), (255,0,0), 3)
                    cv2.line(rgb, (
                    (angular_point[1][0] + angular_point[3][0]) / 2, (angular_point[1][1] + angular_point[3][1]) / 2),
                             (cX, cY), (255, 0, 0), 3)
...
def main():
    try:
        ...
        sub = rospy.Subscriber("/pick_place_tile_vision/object_detect_id", UInt8, code_flag_sub.callback_object_detect_id)
        sub = rospy.Subscriber("/pick_place_tile_vision/desire_detect_id", UInt8, code_flag_sub.callback_desire_detect_id)
