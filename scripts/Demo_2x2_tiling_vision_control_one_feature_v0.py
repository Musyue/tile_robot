...
def main():
    ...
    uvcentral = [355, 91]#sucking central
    First_joint_angular=[]
    calibinfo=[
        0.109982645426,
        0.114476746567,
    ...
    Object_joint_angular_sucking_state = [-110.18,-93.16,-92.82,-82.06,86.68,66.02]
        ...
        try:
            ...
            if len(q_now) != 0:
                ...
                if len(F0.tile_0_buf)!=0 and open_vison_flag_desire ==0:
                    ...
                    MoveUrString=F0.Move_ur(q_pub_now, ace, vel, urt)
                    ur_pub.publish(MoveUrString)
                    feature_error_zero_flag=F0.check_vison_error_is_zero(F0.tile_0_buf[-1][1],uvcentral_place,3)
                    """
                    if error is zero ,we close vision servo
                    """
                    if feature_error_zero_flag:
                ...
                if open_vison_flag_desire ==1 and open_suking_flag_desire==1:
                    ...
                    x_distance = 0.40
                    y_distcane= -0.315
                    q_new_pub = F0.move_ur_to_desire_vertical(x_distance, y_distcane,q_now)
                ...
                if open_move_to_object_flag==1:
                    ...
                    # x_distance = 0.35
                    # q_new_pub=F0.move_ur_to_desire(x_distance,q_now)
                    MoveUrString=F0.Move_ur(Object_joint_angular_sucking_state_new, ace, 0.3, urt)
                    ur_pub.publish(MoveUrString)
                    open_vison_flag = 1
                    open_suking_flag = 0
                    open_move_to_desire_flag=0
